...
Процедура ПриОткрытии(Отказ)
	...
	Если Не ПустаяСтрока(Объект.РасширенныйЛогФайл) Тогда
		ЗаписатьИнформациюВЛог(ТекстСообщения);
	КонецЕсли;
	
	Сообщить("SHORT_CONFIG_NAME:" + Объект.КраткоеИмяКонфигурации);
	Сообщить("CONFIG_VERSION:" + Объект.ВерсияКонфигурации);
	Сообщить("PLATFORM:" + Объект.Платформа);
	
	Если Не ЭтоКонфигурацияНаБазеБСПСервер() И Объект.НужноЗавершитьРаботу Тогда
...
Процедура ЗаписатьИнформациюВЛог(ТекстСообщения)
	...
	Если ПустаяСтрока(ФайлЛога.Путь) Тогда
		...
		Объект.РасширенныйЛогФайл = КатВремФайлов + ?(Прав(КатВремФайлов, 1)=Разделитель, "", Разделитель) + Объект.РасширенныйЛогФайл;
	КонецЕсли;
	ФайлЛога = Новый Файл(Объект.РасширенныйЛогФайл);
